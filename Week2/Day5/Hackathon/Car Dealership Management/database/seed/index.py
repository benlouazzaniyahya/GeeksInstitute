...
from models.your_model import Vehicle, Salesperson, Customer, Sale
...
def seed_database():
    ...
    for data in salespeople_data:
        salesperson = Salesperson(
            ...
            email=data[2],
            phone=data[3],
            hire_date=datetime.strptime(data[4], '%Y-%m-%d').date()
        )
    ...
    # Commit the created records to get their IDs
    db.session.commit()
    ...
    for data in sales_data:
        sale = Sale(
            vehicle_id=data[0],
            customer_id=data[1],
            salesperson_id=data[2],
            sale_date=datetime.strptime(data[3], '%Y-%m-%d').date(),
            sale_price=data[4]
        )
        db.session.add(sale)
